...
// import Header from '@/components/Header'; // Assuming you might have these
...
type Project = {
  title: string;
  description: string;
  technologies: string[]; // Changed to array for better mapping
  liveLink?: string;
  codeLink?: string;
...
const skillCategoriesData: SkillCategory[] = [
  ...
  },
  {
    ...
    skills: [
      ...
      { name: "HTML5 & CSS3", icon: <span className="text-orange-500 text-2xl font-bold mr-0">5</span> }, // Example custom icon
  ...
  },
  {
    ...
    skills: [
      ...
      { name: "AWS (S3, EC2)", icon: <FaAws className="w-6 h-6 text-orange-500" /> },
...
const projectsData: Project[] = [
  {
    ...
    description: "A full-featured online store with user authentication, product listings, cart functionality, and a Stripe payment integration.",
    technologies: ["Next.js", "TypeScript", "Tailwind CSS", "Stripe", "PostgreSQL"],
    liveLink: "#",
    codeLink: "#",
    imageUrl: "https://via.placeholder.com/600x400.png?text=E-commerce+Platform", // Placeholder
  },
  {
    title: "Task Management API",
    description: "A RESTful API for managing tasks, users, and projects, built with Node.js and Express, featuring JWT authentication.",
    technologies: ["Node.js", "Express.js", "MongoDB", "JWT", "Docker"],
    codeLink: "#",
    imageUrl: "https://via.placeholder.com/600x400.png?text=Task+API", // Placeholder
  },
  // Add 1-2 more projects for a 2x2 grid or adjust grid columns
];
...
export default function TechnicalOverviewPage() {
  ...
  return (
    <>
      ...
      <main className="container mx-auto px-4 py-12 md:py-20 bg-gray-50 min-h-screen"> {/* Added bg-gray-50 and min-h-screen */}
        <div className="max-w-6xl mx-auto"> {/* Wider container for more content */}
          
          <div className="text-center mb-16 md:mb-20">
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-black tracking-tight leading-tight font-display">
              Technical Proficiency
            </h1>
            <p className={`mt-4 ${descriptionFontSize} text-gray-600 max-w-2xl mx-auto`}>
          ...
          <section id="skills" className="mb-16 md:mb-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10">
              {skillCategoriesData.map((category) => (
                <div key={category.title} className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                  <h3 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-5 font-display border-b border-gray-300 pb-3">
                    {category.title}
                  </h3>
                  <ul className="space-y-3">
                    {category.skills.map((skill) => (
                      <li key={skill.name} className="flex items-center text-gray-700 hover:text-black transition-colors duration-200">
                        {skill.icon ? 
                          React.cloneElement(skill.icon as React.ReactElement, { className: `w-6 h-6 mr-3 flex-shrink-0 ${skill.icon.props.className || ''}` }) : 
                          <span className="w-6 h-6 mr-3 flex-shrink-0" /> /* Placeholder dot or empty space */
                        }
          ...
          <section id="projects">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              {projectsData.map((project) => (
                <div 
                  key={project.title} 
                  className="bg-white rounded-xl shadow-xl border border-black flex flex-col overflow-hidden group"
                >
                  {project.imageUrl && (
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto flex flex-wrap gap-x-4 gap-y-2 pt-4 border-t border-gray-200">
                      {project.liveLink && (
                        <Link href={project.liveLink} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-500 font-medium hover:underline transition-colors duration-200">
                          Live Demo →
                        </Link>
                      )}
                      {project.codeLink && (
                        <Link href={project.codeLink} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-500 font-medium hover:underline transition-colors duration-200">
                          View Code →
                        </Link>
                      )}
                    </div>
